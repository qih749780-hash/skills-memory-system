...
### 1.5 不想安装？直接把这段发给 agent

把下面这段复制，粘贴给任意 agent，它会立刻按这套系统工作：

```
请使用 skill-memory-system 的方法，在当前工作目录旁边建立项目记忆体系。

规则：
- 新建记忆目录（命名自定，如 memory/ 或 notes/），已存在则追加不覆盖
- 用户原话单独存放，原封不动，不改写不总结
